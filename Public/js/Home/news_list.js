$( function (){
        ...
        $(".better").click(function (){
            ...
            $.post(SITE_URL+"/News/more",{"cata":cata_id,'init':1,'page':1},function (rtn){
                 if (rtn.status==1) {
                  ...
                  }if (rtn.more.end==1) {$(".box-4").hide();}
                  if (!rtn.more.end) {$(".box-4").show();}
                  else{
		...
		$(".skin").click(function (){
           //按浏览时加载更多的新闻
           if($('input').is('#page')) {page=parseInt($('#page').val())+1;if($('input').is("#cata_id")){cata_id=$("#cata_id").val();console.log(page);}}
            ...
            $.post(SITE_URL+"/News/more",{"cata":cata_id,'init':2,'page':page},function (returns){
                 if (returns.status==1) {
                     ...
                     $(returns.more).each(function  (i) {
                          box+='<div class=in><div class="left" ><a href="'+SITE_URL+'/News/news_detail/id/'+returns.more[i]["id"]+'"><img src="' +PUBLIC+'/'+ returns.more[i]["news_picture"] +'" width=300px height=180px></a><a href=#><span class=cata>'+returns.more[i]['cata_id']+'</span> </a></div><div class=right><p class=list><a href="'+ SITE_URL +'/News/news_detail'+returns.more[i]["id"]+'">'+ returns.more[i]["news_title"]+'</a></p><p class=author>'+  subString(returns.more[i]["news_content"],150) +'</p><p class=author>'+ returns.more[i]["author"] +'&nbsp;.&nbsp;'+D(returns.more[i]["add_time"])+'</p></div></div>';
